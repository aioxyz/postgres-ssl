...
if [ -f "$POSTGRES_CONF_FILE" ]; then
  echo "PostgreSQL is already initialized, checking SSL certificates..."
  ...
  echo "PostgreSQL not initialized yet - first run detected"
  echo "Certificates will be generated by init-ssl.sh during initialization"
fi
